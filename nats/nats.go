...
func sanitizeProperties(m map[string]interface{}) map[string]interface{} {
	for _, i := range []string{"MaxReconnect", "MaxPingsOut", "ReconnectBufSize", "SubChanLen"} {
		if v := m[i]; v != nil {
			m[i], _ = v.(int)
		}
	}
	...
	for _, i := range []string{
		...
		if v := m[i]; v != nil {
			if b, ok := v.(bool); ok {
				...
				m[i], _ = strconv.ParseBool(s)
			}
		}
	}

	if v := m["Servers"]; v != nil {
		if s, ok := v.([]string); ok {
			m["Servers"] = s
		} else if s, ok := v.(string); ok {
			m["Servers"] = strings.Split(s, ",")
		}
	}
...
func Factory(natsConfiguration core.CacheProvider, logger *zap.Logger, stale time.Duration) (core.Storer, error) {
	...
	if natsConfiguration.Configuration != nil {
		...
		if bucket, ok := natsConfiguration.Configuration.(map[string]interface{})["kv"]; ok {
	...
	if err != nil {
		logger.Sugar().Error("Impossible to instanciate the Nats DB.", err)
	...
	return &Nats{jsCtx: js, bucket: bucketName, logger: logger, stale: stale}, nil
}
...
func (provider *Nats) MapKeys(prefix string) map[string]string {
	...
	kv, err := provider.jsCtx.KeyValue(provider.bucket)
	if err != nil {
		return keys
	}
...
func (provider *Nats) GetMultiLevel(key string, req *http.Request, validator *core.Revalidator) (fresh *http.Response, stale *http.Response) {
	...
	value, err := kv.Get(core.MappingKeyPrefix + key)
	if err != nil {
...
func (provider *Nats) SetMultiLevel(baseKey, variedKey string, value []byte, variedHeaders http.Header, etag string, duration time.Duration, realKey string) error {
	...
	if err != nil {
		provider.logger.Sugar().Errorf("Impossible to encode the key %s in Nats", variedKey)
	...
	_, err = kv.Put(variedKey, buf.Bytes())
	if err != nil {
	...
	val, e := core.MappingUpdater(variedKey, r, provider.logger, now, now.Add(duration), now.Add(duration+provider.stale), variedHeaders, etag, realKey)
	if e != nil {
		provider.logger.Sugar().Errorf("Impossible to update the mapping key %s in Nats", mappingKey)
		return e
	}
