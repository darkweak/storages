...
// newStorer builds a simplefs instance in a dedicated temporary directory.
// A size of 0 means the in memory cache is unbounded, an empty directorySize
// means the directory is not bounded either.
func newStorer(tb testing.TB, size int, directorySize string) (core.Storer, string) {
	tb.Helper()

	dir := tb.TempDir()

	client, err := simplefs.Factory(core.CacheProvider{
		Path: dir,
	...
	if err != nil {
		tb.Fatalf("impossible to create the simplefs instance: %v", err)
	}

	if err := client.Init(); err != nil {
		tb.Fatalf("impossible to init the simplefs instance: %v", err)
	}

	return client, dir
}

func getBoundedInstance(tb testing.TB, directorySize string) (core.Storer, string) {
	tb.Helper()

	return newStorer(tb, 100, directorySize)
}

// incompressiblePayload returns a payload that lz4 cannot shrink, so the
// on-disk size is predictable. The content is derived from the given seed to
...
func directorySizeOf(tb testing.TB, dir string) int64 {
	tb.Helper()
	...
	if err != nil {
		tb.Fatalf("impossible to read the directory %s: %v", dir, err)
	}
